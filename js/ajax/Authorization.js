...
class Authorization {
    ...
    openModal() {
        if (localStorage.getItem('token') === 'Incorrect username or password' || localStorage.getItem('token') === null){
            let modal = new Modal();
            modal.show('Вхід', 'logSubmit');
            modal.addTitle('Авторизація');
            modal.addInput('вкажіть e-mail', 'Еmail', false, 'email');
            modal.addInput('вкажіть пароль', 'Пароль', true, 'pass');
            this.logIn();
        }
    }
    ...
    checkLogin() {
        if (localStorage.getItem('token') !== 'Incorrect username or password' && localStorage.getItem('token') !== null && localStorage.getItem('token').length < 50){
            this.logInBtn.textContent = 'Створити візит';
